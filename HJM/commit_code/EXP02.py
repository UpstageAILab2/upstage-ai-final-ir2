...
# sparse, dense 앙상블
def ensemble_results(sparse_results, dense_results, size):
    combined_results = {}
    ...
    for hit in sparse_results['hits']['hits']:
        ...
        if doc_id not in combined_results:
            combined_results[doc_id] = {'score': 0, 'source': hit['_source']}
    ...
    # 상위 size 개수만큼 결과 반환
    final_results = [{'id': doc_id, 'score': data['score'], 'source': data['source']['content']} for doc_id, data in sorted_results[:size]]
...
# 사용할 모델을 설정(여기서는 solar-1-mini-chat 모델 사용)
...
eval_rag("../data/eval.jsonl", "../submit/EXP02.csv")